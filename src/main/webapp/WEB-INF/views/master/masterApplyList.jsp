...
<body>
	...
	<div id="page-right-content" class="masterContents">
		<div class="container">
			<div class="row">
				<div class="col-sm-12">
					<h4 class="header-title m-t-0 m-b-20">문의 요청 리스트</h4>
				</div>
			</div>
		</div>
		<br>
				...
				<div class="table-responsive">
					<h5>미완료된 문의요청</h5>
					  <table id="datatable-buttons" class="table table-striped table-bordered" cellspacing="0" width="100%">
						...
						<tbody>
						<c:forEach var="appY" items="${ appYList }">
								<tr id="appYList" onclick="appYList_click(this);">
									<input type="hidden" value="${ appY.appCode }" name="mCode">
									<td>${ appY.appName }</td>
									<td>${ appY.appPhone }</td>
									<td>${ appY.appManager }</td>
									<td>${ appY.appContent }</td>
									<td id="msgDate"><fmt:formatDate value="${ appY.appDate }"
											pattern="yyyy년 MM월 dd일" /></td>
									<td>${ appY.appStatus }</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
			...
			<div class="card-box">
				...
				<div class="table-responsive">
					 <table id="datatable-keytable" class="table table-striped table-bordered">
						...
						<tbody>
							<c:forEach var="appN" items="${ appNList }">
								<tr id="appNList" onclick="appNList_click(this);">
									<input type="hidden" value="${ appN.appCode }" name="appCode">
									<td>${ appN.appName }</td>
									<td>${ appN.appPhone }</td>
									<td>${ appN.appManager }</td>
									<td>${ appN.appContent }</td>
									<td id="msgDate"><fmt:formatDate value="${ appN.appDate }"
											pattern="yy-MM-dd [HH:mm]" /></td>
									<td>${ appN.appStatus }</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
