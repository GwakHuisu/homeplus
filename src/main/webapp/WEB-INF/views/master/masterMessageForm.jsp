                            ...
                            <div class="col-sm-12">
                                <div class="card-box">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover mails m-0 table table-actions-bar">
											...
											<c:forEach var="me" items="${ meList }">
												<tr>
													...
													<c:url var="meReceiveDetail" value="meReceiveDetail.do">
														<c:param name="meCode" value="${ me.meCode }"/>
													</c:url>
													<td id="msgContent">
														<a href="${ meReceiveDetail }" class="text-muted">${ me.meContent }</a>
													</td>
											...
											<tr align="center" height="20" id="pageTable">
												<td colspan="6">
													...
													<c:if test="${ pi.currentPage > 1 }">
														<c:url var="before" value="masterMessageForm.do">
													...
													<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
														...
														<c:if test="${ p ne pi.currentPage }">
															<c:url var="pagination" value="masterMessageForm.do">
													...
													<c:if test="${ pi.currentPage < pi.maxPage }">
														<c:url var="after" value="masterMessageForm.do">
															<c:param name="page" value="${ pi.currentPage + 1 }"/>
															<c:param name="maName" value="${ loginMaster.maName }"/> 
														</c:url>
											...
											<c:if test="${ keyword != null }">
												...
												<td colspan="6">
													...
													<c:if test="${ pi.currentPage > 1 }">
														<c:url var="before" value="messageReceiveSearch.do">
															<c:param name="page"  value="${ pi.currentPage  -1 }"/>
															<c:param name="maName" value="${ loginMaster.maName }"/> 
													...
													<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
														...
														<c:if test="${ p ne pi.currentPage }">
															<c:url var="pagination" value="messageReceiveSearch.do">
																<c:param name="page" value="${ p }"/>
																<c:param name="maName" value="${ loginMaster.maName }"/> 
																<c:param name="category" value="${ category }"/>
																<c:param name="keyword" value="${ keyword }"/>
													...
													<c:if test="${ pi.currentPage < pi.maxPage }">
														<c:url var="after" value="messageReceiveSearch.do">
															<c:param name="page" value="${ pi.currentPage + 1 }"/>
															<c:param name="maName" value="${ loginMaster.maName }"/> 
															<c:param name="category" value="${ category }"/>
															<c:param name="keyword" value="${ keyword }"/>
                                        ...
                                        <br>
                                        <form action="messageReceiveSearch.do" method="GET">
