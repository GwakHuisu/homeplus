				...
				<div class="card-box">
					...
					<div class="table-responsive">
						<table class="table table-hover mails m-0 table table-actions-bar">
							...
							<tbody>
								<c:forEach var="rw" items="${list}">
									<tr
										onclick="document.location.href='rwdetail.do?bbCode=${rw.bbCode}'">
										<td align="center">${rw.bbCode}</a></td>
										<td align="center">${rw.bbDcategory}</td>
										<td align="center">${rw.bbTitle}</td>
										<td align="center">${rw.bbContent}</td>
										<c:choose>
											<c:when test="${rw.bbStatus eq 'N'.charAt(0) }">
												<td align="center">처리전</td>
											</c:when>
											<c:when test="${rw.bbStatus eq 'L'.charAt(0) }">
					...
					<div id="pagenation">
						<ul class="pagination pagination-split">
							<c:if test="${searchValue eq null }">
								<c:set var="loc" value="rwlist.do" />
							</c:if>
							<c:if test="${searchValue ne null }">
								<c:set var="loc" value="rwsearch.do" />
							</c:if>
							...
							<c:if test="${pi.currentPage < pi.maxPage }">
								<c:url var="after" value="rwlist.do">
									<c:param name="page" value="${pi.currentPage + 1}" />
					...
					<div id="serachArea" align="center">
						<form action="rwsearch.do" name="searchForm" method="get">
						...
						<script>
							...
							function searchBulletinBoard() {
								...
								location.href = "rwsearch.do?searchCondition="
