	...
	<div id="total">
		...
		<div id="main">
			<div class="ingu">
				...
				<div id="headTitle">				
					<h4>
						<a id="title" href="myReply.do">내 댓글</a>
					</h4>
					...
					<table class="sub_news" border="1">
						<thead>
							<tr>
								...
								<th width="500" id="contentTitle">내용</th>
								<th width="100" id="contentTitle">작성글</th>
						...
						<tbody>
							...
							<c:if test="${!empty brList }">
								<c:forEach var="b" items="${brList }" varStatus="status">
									<c:url var="myBoardDetail" value="myBoardDetail.do">
										<c:param name="b_code" value="${b.bCode }" />
										<c:param name="curPage" value="${pi.currentPage }" />
										<c:param name="b_name" value="${b.bName }" />
										<c:param name="check" value="3" />
									</c:url>
									
									<c:if test="${fn:length(b.bTitle) > 10 }">
										<c:set var="bTitleShot" value="${fn:substring(b.bTitle, 0, 10) }..." />								
									</c:if>
									<c:if test="${fn:length(b.bTitle) <= 10 }">
										<c:set var="bTitleShot" value="${b.bTitle }" />	
									</c:if>
									
									<tr>
										<td>${b.rCode }</td>
										<td><a href="${myBoardDetail}">${b.rContent}</a></td>
										<td onMouseOver="sample_string_view(true, 'hideText${b.rCode}');" onMouseOut="sample_string_view(false,'hideText${b.rCode}');">${bTitleShot }</td>
										<td>${b.rCreateDate }${status.index }</td>
									</tr>
																	
									
									<div id="hideText${b.rCode }" style="display:none;left:62%;position:absolute;top:${status.index * 50 + 220}px;">
													${b.bName } / ${b.bContent }						
									</div>
									
									<script>								
										function sample_string_view(flag, objName) { 
...
<div id="line_foot">
	...
	<div id="searchArea" style="text-align:left;">
		<form action="searchMyReply.do" name="searchForm" method="post">
			<select id="searchCondition" name="searchCondition">
				<option value="all" <c:if test="${ search.searchCondition == 'all' }">selected</c:if>>전체</option>
				<option value="writer" <c:if test="${ search.searchCondition == 'writer' }">selected</c:if>>내용</option>
				<option value="title"<c:if test="${ search.searchCondition == 'title' }">selected</c:if>>글제목</option>
				<option value="content"<c:if test="${ search.searchCondition == 'content' }">selected</c:if>>글내용</option>
			</select> 
